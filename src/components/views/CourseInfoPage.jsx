...
import {faArrowRight,faPlay,faComment,faArrowLeft,faChevronLeft,faBars} from '@fortawesome/free-solid-svg-icons'
...
export default function CourseInfoPage() {
  ...
  return (
      ...
      <Container90>
        <div className="flex justify-between content">
          ...
          <div className="items-center hidden gap-2 left lg:flex">
            ...
            <Button variant="contained" disableElevation sx={{borderRadius:0}}>
              <div className="flex items-center gap-1">
                <p>المحاضرة التالية</p>
                <div className="flex items-center icon"><FontAwesomeIcon icon={faChevronLeft} /></div>
              </div>
            </Button>
          </div>
          <div className="flex items-center gap-1 left lg:hidden ">
          <button onClick={()=>setWatchPopupOpen(true)} className="flex items-center justify-center w-6 h-6 text-xl bg-gray-100 rounded-full text-primary-500 hover:bg-gray-200"> 
      ...
      <div className= "flex flex-col gap-2 px-2 my-3 text-gray-900 lg:flex-row main sm:px-0">
        <div className="lg:basis-3/4 content">
          <div className="video">
            ...
            <h1 className="my-2 text-2xl font-normal md:font-bold title">1.متوسط التغير</h1>
    ...
    </Container90>
    {
      watchPopupOpen?
      <PopupLayout>
        <div className="relative z-10 w-10/12 py-5 bg-gray-50">
          <button onClick={()=>setWatchPopupOpen(false)} className="absolute top-[-1rem] right-[-1rem] text-gray-white bg-primary-500 p-2 w-6 h-6 rounded-full flex items-center justify-center text-xl hover:bg-primary-600">
              X
            </button>
            <WatchMenu/>
          </div>
